...
Panel.tabHeaders = {}     -- group headers
...
function Panel:RegisterTab(key, text, builder, order, group)
    self.tabs[key] = {
        text    = text,
        value   = key,
        builder = builder,
...
local function CreatePanel()
    ...
    titleText:SetText("|cff1784d1DragonUI-CF|r |cffaaaaaa3.6.8|r")
...
local function RegisterFallbackMrKrisTab()
    if Panel.tabs["mrkrissatan"] then
        return
    end
    ...
    local function RefreshMinimap()
        ...
        elseif addon.MinimapModule and addon.MinimapModule.UpdateSettings then
            addon.MinimapModule:UpdateSettings()
        end
    end

    local function BuildFallbackTab(scroll)
    ...
    Panel:RegisterTab("mrkrissatan", "Additions", BuildFallbackTab, 100, "MrKrisSatan's Additions")
end
...
local function BuildTabButtons()
    ...
    for _, header in pairs(Panel.tabHeaders) do
        header:Hide()
        header:SetParent(nil)
    end
    wipe(Panel.tabHeaders)

    local strip = Panel.frame.tabStrip
    local topY = -6
    local groupedKeys = {}

    local function CreateTabButton(key, point, relativePoint, xOff, yOff)
        local tabInfo = Panel.tabs[key]
        local btn = CreateFrame("Button", nil, strip)
        btn:SetSize(136, 22)
        btn:SetPoint(point, strip, relativePoint, xOff, yOff)
    ...
    for _, key in ipairs(Panel.tabOrder) do
        local tabInfo = Panel.tabs[key]
        if tabInfo.group == "default" then
            CreateTabButton(key, "TOPLEFT", "TOPLEFT", 2, topY)
            topY = topY - 24
        else
            table.insert(groupedKeys, key)
        end
    end

    if #groupedKeys > 0 then
        local bottomY = 8
        for index = #groupedKeys, 1, -1 do
            local key = groupedKeys[index]
            CreateTabButton(key, "BOTTOMLEFT", "BOTTOMLEFT", 2, bottomY)
            bottomY = bottomY + 24
        end

        local firstGroupedKey = groupedKeys[1]
        local firstGroupedButton = Panel.tabButtons[firstGroupedKey]
        local groupName = Panel.tabs[firstGroupedKey] and Panel.tabs[firstGroupedKey].group
        if firstGroupedButton and groupName and groupName ~= "default" then
            local header = strip:CreateFontString(nil, "OVERLAY")
            SetSafeFont(header, 11, "OUTLINE")
            header:SetPoint("BOTTOMLEFT", firstGroupedButton, "TOPLEFT", 6, 8)
            header:SetWidth(124)
            header:SetJustifyH("LEFT")
            header:SetTextColor(1, 0.82, 0, 1)
            header:SetText(groupName)
            Panel.tabHeaders[firstGroupedKey] = header
        end
    end
end
...
function Panel:SelectTab(key)
    ...
    if self.frame and self.frame.DragonUIMinimapShapeToggle then
        self.frame.DragonUIMinimapShapeToggle:Hide()
    end
