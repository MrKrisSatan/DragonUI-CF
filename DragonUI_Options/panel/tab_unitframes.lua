...
local partyRaidStyleVariantValues = {
    classic = "Classic DragonUI",
    new     = "New Portraitless",
}
...
local function BuildPartySection(scroll)
    ...
    C:AddToggle(s, {
        ...
        desc = "Switch party frames to Blizzard's compact raid-style layout. Move them with the Party mover in Editor Mode.",
        dbPath = "unitframe.party.useRaidStyle",
        callback = function(value)
            if addon.SetPartyRaidStyleEnabled then
                addon:SetPartyRaidStyleEnabled(value)
            else
                refreshParty()
            end
            RefreshUnitFramesTabAfterToggle(refreshParty)
        end,
    })

    C:AddDropdown(s, {
        label = "Raid-Style Layout",
        desc = "Choose between the classic DragonUI compact party look and the newer portraitless bars.",
        dbPath = "unitframe.party.raidStyleVariant",
        values = partyRaidStyleVariantValues,
        callback = refreshParty,
    })

    C:AddToggle(s, {
        label = "Portraitless Party Bars",
        desc = "Hide party portraits and use the newer portraitless bar layout for regular party frames.",
        dbPath = "unitframe.party.portraitless",
        callback = refreshParty,
    })
