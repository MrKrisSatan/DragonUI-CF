...
local DRAGONUI_MINIMAP_SQUARE_MASK = "Interface\\Buttons\\WHITE8X8"
local DRAGONUI_MINIMAP_BORDER_TEXTURE = "Interface\\AddOns\\DragonUI\\assets\\uiminimapborder.tga"
local SOLID_TEXTURE = "Interface\\Buttons\\WHITE8X8"

local ADDON_ORBIT_RADIUS = 15
local ADDON_BUTTON_TRAY_BUTTON_SIZE = 20
local ADDON_BUTTON_TRAY_SPACING = 4
local ADDON_BUTTON_TRAY_PADDING = 4
local ADDON_BUTTON_TRAY_MAX_COLUMNS = 6
local SQUARE_BORDER_PADDING = 0
local SQUARE_BORDER_EDGE_SIZE = 2
local SQUARE_BORDER_GLOW_SIZE = 4
local SQUARE_BORDER_MARKER_ARM = 14
local SQUARE_BORDER_MARKER_THICKNESS = 3
local SQUARE_BORDER_MARKER_OFFSET = 0

local UpdateSquareBorderStyle

local function GetAddonOrbitRadius()
    local settings = addon and addon.db and addon.db.profile and addon.db.profile.minimap
    return (settings and settings.addon_button_orbit) or ADDON_ORBIT_RADIUS
end

local function GetMinimapButtonTrayConfig()
    local widgets = addon and addon.db and addon.db.profile and addon.db.profile.widgets
    if not widgets then return nil end
    return widgets.minimap_buttons
end
...
local function UpdateMinimapCircleSize()
    ...
    if settings and settings.square then
        if Minimap.Circle then
            Minimap.Circle:Hide()
        end

        if Minimap.SquareBorder then
            UpdateSquareBorderStyle()
        end
        return
    end

    if Minimap.SquareBorder then
        Minimap.SquareBorder:Hide()
    end

    if not Minimap.Circle then return end
...
local function UpdateMinimapMaskForRotation()
    ...
    local settings = addon and addon.db and addon.db.profile and addon.db.profile.minimap
    local desiredMask = (settings and settings.square) and DRAGONUI_MINIMAP_SQUARE_MASK or DRAGONUI_MINIMAP_MASK
...
local function EnsureSquareBorderMarkers()
    if not Minimap or not Minimap.SquareBorder then return end

    local squareBorder = Minimap.SquareBorder
    if squareBorder._dragonMarkersCreated then
        return
    end

    squareBorder.TopAccent = squareBorder:CreateTexture(nil, "ARTWORK")
    squareBorder.TopAccent:SetTexture(SOLID_TEXTURE)
    squareBorder.TopAccent:SetVertexColor(0.95, 0.80, 0.30, 0.95)

    squareBorder.TopGlow = squareBorder:CreateTexture(nil, "BORDER")
    squareBorder.TopGlow:SetTexture(SOLID_TEXTURE)
    squareBorder.TopGlow:SetVertexColor(0.95, 0.80, 0.30, 0.18)

    squareBorder._dragonMarkersCreated = true
end

UpdateSquareBorderStyle = function()
    if not Minimap or not Minimap.SquareBorder then return end

    EnsureSquareBorderMarkers()

    local squareBorder = Minimap.SquareBorder
    local squareSize = math.max(Minimap:GetWidth(), Minimap:GetHeight()) + SQUARE_BORDER_PADDING

    squareBorder:ClearAllPoints()
    squareBorder:SetPoint("CENTER", Minimap, "CENTER", 0, 0)
    squareBorder:SetSize(squareSize, squareSize)
    squareBorder:SetBackdropBorderColor(0.91, 0.76, 0.26, 0.95)
    if squareBorder.Glow then
        squareBorder.Glow:ClearAllPoints()
        squareBorder.Glow:SetPoint("TOPLEFT", squareBorder, "TOPLEFT", 1, -1)
        squareBorder.Glow:SetPoint("BOTTOMRIGHT", squareBorder, "BOTTOMRIGHT", -1, 1)
        squareBorder.Glow:SetBackdropBorderColor(0.96, 0.82, 0.30, 0.16)
        squareBorder.Glow:Show()
    end

    if squareBorder.TopAccent then
        squareBorder.TopAccent:ClearAllPoints()
        squareBorder.TopAccent:SetPoint("TOPLEFT", squareBorder, "TOPLEFT", 3, -1)
        squareBorder.TopAccent:SetPoint("TOPRIGHT", squareBorder, "TOPRIGHT", -3, -1)
        squareBorder.TopAccent:SetHeight(2)
        squareBorder.TopAccent:Show()
    end

    if squareBorder.TopGlow then
        squareBorder.TopGlow:ClearAllPoints()
        squareBorder.TopGlow:SetPoint("TOPLEFT", squareBorder, "TOPLEFT", 3, -1)
        squareBorder.TopGlow:SetPoint("TOPRIGHT", squareBorder, "TOPRIGHT", -3, -1)
        squareBorder.TopGlow:SetHeight(5)
        squareBorder.TopGlow:Show()
    end
...
local function ReplaceBlizzardFrame(frame)
    ...
    if not isHybridMode then
        ...
        if not Minimap.Circle then
            ...
            Minimap.Circle:SetTexture(DRAGONUI_MINIMAP_BORDER_TEXTURE)
        end
        if not Minimap.SquareBorder then
            local squareBorder = CreateFrame("Frame", nil, Minimap)
            squareBorder:SetFrameStrata("MEDIUM")
            squareBorder:SetFrameLevel(Minimap:GetFrameLevel() + 6)
            squareBorder:SetBackdrop({
                edgeFile = "Interface\\ChatFrame\\ChatFrameBackground",
                edgeSize = SQUARE_BORDER_EDGE_SIZE,
                insets = { left = 0, right = 0, top = 0, bottom = 0 },
            })
            squareBorder:SetBackdropBorderColor(0.91, 0.76, 0.26, 0.95)

            local glow = CreateFrame("Frame", nil, squareBorder)
            glow:SetFrameLevel(squareBorder:GetFrameLevel() - 1)
            glow:SetBackdrop({
                edgeFile = "Interface\\ChatFrame\\ChatFrameBackground",
                edgeSize = SQUARE_BORDER_GLOW_SIZE,
                insets = { left = 0, right = 0, top = 0, bottom = 0 },
            })
            glow:SetBackdropBorderColor(0.96, 0.82, 0.30, 0.16)
            squareBorder.Glow = glow

            squareBorder:Hide()
            Minimap.SquareBorder = squareBorder
        end
...
local function GetAllMinimapButtons()
    ...
    ScanFrame(MinimapModule.addonButtonTray)
    
    return buttons
end

local function EnsureAddonButtonTray()
    if MinimapModule.addonButtonTray then
        return MinimapModule.addonButtonTray
    end

    local tray = addon.CreateUIFrame(152, 28, "MinimapButtons")
    tray:SetClampedToScreen(true)
    if tray.editorText then
        tray.editorText:SetText("Minimap Buttons")
    end

    local cfg = GetMinimapButtonTrayConfig()
    if cfg then
        tray:ClearAllPoints()
        tray:SetPoint(cfg.anchor or "TOPRIGHT", UIParent, cfg.anchor or "TOPRIGHT", cfg.posX or -7, cfg.posY or -245)
    else
        tray:SetPoint("TOPRIGHT", UIParent, "TOPRIGHT", -7, -245)
    end

    MinimapModule.addonButtonTray = tray

    if addon.RegisterEditableFrame then
        addon:RegisterEditableFrame({
            name = "minimap_buttons",
            frame = tray,
            configPath = {"widgets", "minimap_buttons"},
            editorVisible = function()
                return true
            end,
            onHide = function()
                if MinimapModule.UpdateAddonButtonTray then
                    MinimapModule.UpdateAddonButtonTray()
                end
            end,
...
local function UpdateAddonButtonTray()
    if not IsMinimapSystemActive() then return end

    local tray = EnsureAddonButtonTray()
    ...
    local columns = math.max(1, math.min(ADDON_BUTTON_TRAY_MAX_COLUMNS, count))
    local rows = math.max(1, math.ceil(count / ADDON_BUTTON_TRAY_MAX_COLUMNS))
    local width = (columns * ADDON_BUTTON_TRAY_BUTTON_SIZE) + ((columns - 1) * ADDON_BUTTON_TRAY_SPACING) + (ADDON_BUTTON_TRAY_PADDING * 2)
    local height = (rows * ADDON_BUTTON_TRAY_BUTTON_SIZE) + ((rows - 1) * ADDON_BUTTON_TRAY_SPACING) + (ADDON_BUTTON_TRAY_PADDING * 2)

    tray:SetSize(math.max(width, 80), math.max(height, 28))

    for index, child in ipairs(visibleButtons) do
        if child:GetParent() ~= tray then
            if child.DragonUI_MinimapButtonOriginalParent == nil then
                child.DragonUI_MinimapButtonOriginalParent = child:GetParent()
            end
            if child.DragonUI_MinimapButtonOriginalScale == nil then
                child.DragonUI_MinimapButtonOriginalScale = child:GetScale()
            end
            child:SetParent(tray)
        end

        local col = (index - 1) % ADDON_BUTTON_TRAY_MAX_COLUMNS
        local row = math.floor((index - 1) / ADDON_BUTTON_TRAY_MAX_COLUMNS)

        child:ClearAllPoints()
        child:SetPoint("TOPLEFT", tray, "TOPLEFT",
            ADDON_BUTTON_TRAY_PADDING + (col * (ADDON_BUTTON_TRAY_BUTTON_SIZE + ADDON_BUTTON_TRAY_SPACING)),
            -ADDON_BUTTON_TRAY_PADDING - (row * (ADDON_BUTTON_TRAY_BUTTON_SIZE + ADDON_BUTTON_TRAY_SPACING)))
        child:SetScale(1)
        child:SetSize(ADDON_BUTTON_TRAY_BUTTON_SIZE, ADDON_BUTTON_TRAY_BUTTON_SIZE)
        child:SetFrameStrata("MEDIUM")
        child:Show()
    end

    if count > 0 or (addon.EditorMode and addon.EditorMode:IsActive()) then
        tray:Show()
    else
        tray:Hide()
    end
...
local function UpdateAddonButtonOrbit()
    UpdateAddonButtonTray()
end

-- Expose for options to trigger
MinimapModule.ApplySkinsToAllMinimapButtons = ApplySkinsToAllMinimapButtons
MinimapModule.UpdateAddonButtonOrbit = UpdateAddonButtonOrbit
MinimapModule.UpdateAddonButtonTray = UpdateAddonButtonTray
...
function addon:GrabMinimapButtons()
    if not MinimapModule.isEnabled then
        return
    end
