...
local function ApplyFatManaBar()
    ...
    local powerType = UnitPowerType("player")  -- 0=mana, 1=rage, 3=energy, 6=runic

    if not fatMode then
        -- Normal mode: position power bar(s) tightly next to portrait
        ...
        if hasVehicleUI then
            ...
            -- TIGHT POSITION NEXT TO PORTRAIT for all power types
            if powerType == 1 then      -- Rage
                PlayerFrameManaBar:SetPoint('LEFT', PlayerPortrait, 'RIGHT', -3, -8)
            elseif powerType == 3 then  -- Energy
                PlayerFrameManaBar:SetPoint('LEFT', PlayerPortrait, 'RIGHT', -3, -19)
            else                        -- Mana or others
                PlayerFrameManaBar:SetPoint('LEFT', PlayerPortrait, 'RIGHT', -3, -16.5)
            end
        end
    ...
    -- Fat mode (your existing fat logic remains unchanged)
    local width, height, hidden = GetFatManaConfig()
    if hidden then
        PlayerFrameManaBar:Hide()
        if Module.fatManaFrame then Module.fatManaFrame:SetSize(1, 1) end
        return
    end
    ...
    if not Module.fatManaRegistered and addon.RegisterEditableFrame then
        addon:RegisterEditableFrame({
            ...
            onHide = function() ApplyFatManaBar() end,
            module = Module
        })
    ...
    for index = 1, 6 do
        ...
        if button then
            ...
            if index > 1 then
                ...
                button:SetPoint('CENTER', PlayerFrame, 'BOTTOM', -10, 7)
            end
            UpdateRune(button)
    ...
    if decorationType ~= "none" and not inVehicle then
        ...
        if fatMode then
            ...
            if pConfig and pConfig.classPortrait and not IsInVehicle() then
                ...
                UF.HideAnimatedPortrait(dragonFrame.PortraitOverlayTexture, true)
            else
                SetPortraitTexture(dragonFrame.PortraitOverlayTexture, "player")
                UF.UpdateAnimatedPortrait("player", dragonFrame.PortraitOverlayTexture)
            end
...
local function RestorePlayerPortraitTexture()
    UF.HideAnimatedPortrait(PlayerPortrait, true)
    local dragonFrame = _G["DragonUIUnitframeFrame"]
...
local function GetOrCreateAlternateResourceOverlay()
    if Module.alternateResourceOverlay then
        return Module.alternateResourceOverlay
    end

    local overlay = CreateFrame("StatusBar", nil, PlayerFrame)
    overlay:SetFrameStrata(PlayerFrame:GetFrameStrata())
    ...
    borderTex:SetVertexColor(0.82, 0.67, 0.18, 1)
    overlay.border = borderTex

    Module.alternateResourceOverlay = overlay
    return overlay
end

local function RefreshAlternateResourceOverlay()
    local alternateManaBar = _G.PlayerFrameAlternateManaBar
    local overlay = Module.alternateResourceOverlay
    if not alternateManaBar or not overlay then
        return
    end

    if UnitHasVehicleUI("player") or not alternateManaBar:IsShown() then
        overlay:Hide()
        return
    end

    local minValue, maxValue = alternateManaBar:GetMinMaxValues()
    local value = alternateManaBar:GetValue() or 0
    overlay:SetMinMaxValues(minValue or 0, maxValue or 0)
    overlay:SetValue(value)

    local r, g, b = 1, 0.82, 0
    if alternateManaBar.GetStatusBarColor then
        local cr, cg, cb = alternateManaBar:GetStatusBarColor()
...
local function ApplyPlayerBarTextureInsets()
    local function nudgeTexture(bar, leftInset, rightInset)
        ...
        texture:SetPoint("TOPLEFT", bar, "TOPLEFT", leftInset or 0, 0)
        texture:SetPoint("BOTTOMLEFT", bar, "BOTTOMLEFT", leftInset or 0, 0)
        texture:SetPoint("TOPRIGHT", bar, "TOPRIGHT", rightInset or 0, 0)
        texture:SetPoint("BOTTOMRIGHT", bar, "BOTTOMRIGHT", rightInset or 0, 0)
    end

    nudgeTexture(PlayerFrameHealthBar, -8, 0)
    nudgeTexture(PlayerFrameManaBar, -8, 0)
    nudgeTexture(Module.alternateResourceOverlay, -10, 0)
end

local function MovePlayerClasslessBarText(bar)
...
local function MovePlayerClasslessResourceBars()
    if not PlayerFrameManaBar then
        return
    end

    local bars = {
        _G.PlayerFrameClasslessEnergyBar,
        _G.PlayerFrameClasslessRageBar,
    }
    ...
    for _, bar in ipairs(bars) do
        if bar then
            shown = shown + 1
            local yOffset = -2 - ((shown - 1) * 5)

            bar:ClearAllPoints()
            bar:SetPoint("TOPLEFT", PlayerFrameManaBar, "BOTTOMLEFT", 0, yOffset)
            bar:SetPoint("TOPRIGHT", PlayerFrameManaBar, "BOTTOMRIGHT", 0, yOffset)
            bar:SetHeight(4)
            bar:SetFrameLevel(PlayerFrameManaBar:GetFrameLevel() + 1)
            ...
            MovePlayerClasslessBarText(bar)
        end
    end
end

local function LayoutPlayerAlternateResourceBar()
    local alternateManaBar = _G.PlayerFrameAlternateManaBar
    if not alternateManaBar then
        return
    end

    alternateManaBar:SetAlpha(0)
    alternateManaBar:Hide()
    local originalTexture = alternateManaBar.GetStatusBarTexture and alternateManaBar:GetStatusBarTexture()
    if originalTexture then
        originalTexture:SetAlpha(0)
        originalTexture:Hide()
    end
    if Module.alternateResourceOverlay then
        Module.alternateResourceOverlay:Hide()
    end

    MovePlayerClasslessResourceBars()
end
...
local function UpdatePlayerClassPortrait()
    ...
    if not IsInVehicle() then
        UF.UpdateAnimatedPortrait("player", PlayerPortrait)
    end
    ...
    if dragonFrame and dragonFrame.PortraitOverlayTexture
       ...
       and dragonFrame.PortraitOverlay:IsShown() then
        ...
        if pConfig and pConfig.classPortrait and not IsInVehicle() and portraitUnit ~= "vehicle" then
            ...
            SetPortraitTexture(dragonFrame.PortraitOverlayTexture, portraitUnit)
            if portraitUnit ~= "vehicle" then
                UF.UpdateAnimatedPortrait(portraitUnit, dragonFrame.PortraitOverlayTexture)
            else
                UF.HideAnimatedPortrait(dragonFrame.PortraitOverlayTexture, true)
            end
        end
...
local function ChangePlayerframe()
    ...
    elseif fatMode then
        ...
        PlayerFrameHealthBar:SetPoint('LEFT', PlayerPortrait, 'RIGHT', -3, -HP_OFFSET)
        ...
        PlayerFrameHealthBar:SetPoint('LEFT', PlayerPortrait, 'RIGHT', -3, 0)
        PlayerFrameHealthBar:SetFrameLevel(PlayerFrame:GetFrameLevel() + 1)
        PlayerFrameManaBar:SetFrameLevel(PlayerFrame:GetFrameLevel() + 1)
    end
    ...
    ApplyFatManaBar()
    LayoutPlayerAlternateResourceBar()
...
local function InitializePlayerFrame()
    ...
    local alternateManaBar = _G.PlayerFrameAlternateManaBar
    if alternateManaBar and not Module._alternateResourceOverlayHooked then
        alternateManaBar:HookScript('OnValueChanged', RefreshAlternateResourceOverlay)
        alternateManaBar:HookScript('OnShow', RefreshAlternateResourceOverlay)
        alternateManaBar:HookScript('OnHide', function()
            if Module.alternateResourceOverlay then
                Module.alternateResourceOverlay:Hide()
            end
        end)
        Module._alternateResourceOverlayHooked = true
    end
    ...
    if _G.PlayerFrameClasslessEnergyBar and not Module._playerClasslessEnergyHooked then
        _G.PlayerFrameClasslessEnergyBar:HookScript('OnShow', MovePlayerClasslessResourceBars)
        _G.PlayerFrameClasslessEnergyBar:HookScript('OnValueChanged', MovePlayerClasslessResourceBars)
        Module._playerClasslessEnergyHooked = true
    end

    if _G.PlayerFrameClasslessRageBar and not Module._playerClasslessRageHooked then
        _G.PlayerFrameClasslessRageBar:HookScript('OnShow', MovePlayerClasslessResourceBars)
        _G.PlayerFrameClasslessRageBar:HookScript('OnValueChanged', MovePlayerClasslessResourceBars)
        Module._playerClasslessRageHooked = true
    end
