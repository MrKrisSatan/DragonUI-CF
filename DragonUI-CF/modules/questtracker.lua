...
local function UpdateQuestTrackerCollapseButtonAnchor(frame)
    if not frame or not WatchFrameCollapseExpandButton then
        return
    end
...
local function ReplaceBlizzardFrame(frame)
    ...
    if not watchFrameAttached then
        ...
        UpdateQuestTrackerCollapseButtonAnchor(frame)
...
function QuestTrackerModule:Initialize()
    ...
    self.questTrackerFrame:SetSize(230, 32)
...
function QuestTrackerModule:ShowEditorTest()
    if self.questTrackerFrame then
        ...
        UpdateQuestTrackerCollapseButtonAnchor(self.questTrackerFrame)
