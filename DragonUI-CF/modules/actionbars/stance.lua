...
local function SetStanceButtonsMouseEnabled(enabled)
...
local function RefreshStanceEditorOverlay()
    local editorOverlay = StanceModule.frames.editorOverlay
    if not editorOverlay or not anchor then return end

    local stanceConfig = GetStanceConfig()
    local btnSize = stanceConfig.button_size or 31
    editorOverlay:SetSize(PETBAR_EDITOR_WIDTH, btnSize)
    ...
    if addon.EditorMode and addon.EditorMode:IsActive() then
        SetStanceButtonsMouseEnabled(false)
    else
        SetStanceButtonsMouseEnabled(true)
    end
end
...
local function OnEvent(self,event,...)
	...
	elseif event == 'PLAYER_ENTERING_WORLD' then
        ...
        if event == 'LEARNED_SPELL' or event == 'LEARNED_SPELL_IN_TAB' or event == 'CHARACTER_POINTS_CHANGED' or event == 'SPELLS_CHANGED' then
            stancebutton_setup();
            stancebutton_position();
            RefreshStanceEditorOverlay();
