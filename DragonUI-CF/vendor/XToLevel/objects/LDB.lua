...
XToLevel.LDB = 
{
    ...
    textTags = {
		...
		-- New Soul Ashes Tags
		[14] = { tag = "sph", label = 'init', value = '~', color = nil, },
        [15] = { tag = "sp_total", label = 'init', value = '~', color = nil, },
    },
	...
	---
    -- Generates the text pattern string based on current settings and game state.
    -- This is an expensive function and should only be called when settings change.
    GeneratePattern = function(self)
        if ((XToLevel.Player.level < XToLevel.Player.maxLevel) or XToLevel.Pet.isActive or XToLevel.Pet.hasBeenActive) then
            ...
            -- Free resources
            showPlayer = nil; showPet = nil; playerProgress = nil; playerProgressColor = nil; petProgress = nil; petProgressColor = nil;
            useColors = nil; isFirst = nil; sPos = nil; ePost = nil; value = nil; rest = nil; attributes = nil; out = nil;
	...
	---
    -- Calls GeneratePattern and stores the result in a cache for quick access.
    BuildAndCachePattern = function(self)
        self.cachedPattern = self:GeneratePattern()
    end,
        
    ---
    -- Update LDB text by replacing placeholders in the cached pattern.
    -- This is a lightweight function designed to be called frequently.
    Update = function(self)
        ...
        if sConfig.ldb.showText then
            local pattern = self.cachedPattern;
            ...
            if (XToLevel.Player.level < XToLevel.Player.maxLevel) then
				...
				-- Soul Ashes Injection
				if XToLevel.Player.soulData.active then
					local spTotal, spKill, spQuest = XToLevel.Player:GetSoulAshesPerHour()
					local sessTotal = XToLevel.Player.soulData.sessionTotal
					
					pattern = string.gsub(pattern, '%$%$sph%$%$', XToLevel.Lib:NumberFormat(spTotal))
					pattern = string.gsub(pattern, '%$%$sptotal%$%$', XToLevel.Lib:NumberFormat(sessTotal))
				else
					pattern = string.gsub(pattern, '%$%$sph%$%$', "")
					pattern = string.gsub(pattern, '%$%$sptotal%$%$', "")
				end
