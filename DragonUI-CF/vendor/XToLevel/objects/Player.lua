...
XToLevel.Player = {
	...
	Initialize = function(self)
		...
		-- Initialize Soul Ashes Tracking
		if EbonholdPlayerRunData then
			if EbonholdPlayerRunData.soulPoints then
				self.soulData.active = true
				self.soulData.startAmount = EbonholdPlayerRunData.soulPoints
				self.soulData.currentAmount = EbonholdPlayerRunData.soulPoints
				self.soulData.startTime = time()
			end
		end
	end,
	...
	UpdateTimer = function(self)
		...
		local mode, timeToLevel, timePlayed, xpPerHour, totalXP = self:GetTimerData()
		
		if timeToLevel and timeToLevel > 0 then
			XToLevel.Average:UpdateTimer(timeToLevel)
		else
	...
	--- Returns details about the estimated time remaining based on the current mode.
	-- @return mode, timeToLevel, timePlayed, xpPerHour, totalXP
	GetTimerData = function(self)
		local mode, timeToLevel, timePlayed, xpPerHour, totalXP, xpPerSecond
		local currentMode = sConfig.timer.mode
		
		-- Mode 3: Kill Range
		if currentMode == 3 then
			local killRangeTime, killRangeXp
			timePlayed, totalXP, xpPerSecond = self:GetKillRangeTimerData()
			if timePlayed and totalXP and xpPerSecond then
				mode = 3
			else
				-- Fallback to session if kill range fails
				currentMode = 1
			end
		end
		...
		if currentMode == 1 then
			if type(sData.player.timer.start) == "number" and type(sData.player.timer.total) == "number" and sData.player.timer.total > 0 then
				...
				-- Fallback to level if session fails
				currentMode = 2
			end
		end
		
		-- Mode 2: Level
		if currentMode == 2 and sConfig.timer.allowLevelFallback then
			if XToLevel.Player.timePlayedLevel then
				mode = 2
		...
		if mode and xpPerSecond and xpPerSecond > 0 then
			xpPerHour = ceil(xpPerSecond * 3600)
			timeToLevel = (self.maxXP - self.currentXP) / xpPerSecond
		else
			-- No valid data found for any mode
			mode, timeToLevel, timePlayed, xpPerHour, totalXP = nil, 0, 0, nil, nil
		end

		return mode, timeToLevel, timePlayed, xpPerHour, totalXP
	end,

	--- Calculates timer data based on recent kills.
	-- @return timePlayed, totalXP, xpPerSecond
	GetKillRangeTimerData = function(self)
		local killsToScan = sConfig.averageDisplay.playerKillListLength
		if #sData.player.killList < 2 or #sData.player.killList < killsToScan then
		...
		for i = 1, killsToScan do
			local killData = sData.player.killList[i]
			if killData and killData.xp and killData.time then
				totalXP = totalXP + killData.xp
				if i == 1 then
					lastTime = killData.time
				end
				if i == killsToScan then
					firstTime = killData.time
				end
			else
				-- Old data without timestamp, cannot use this mode
				return nil
			end
		end

		local duration = lastTime - firstTime
		if duration <= 0 then return nil -- Avoid division by zero or nonsensical data
		end

		local xpPerSecond = totalXP / duration
		return duration, totalXP, xpPerSecond
	end,
	...
	AddKill = function(self, xpGained, mobName)
		...
		self.killAverage = nil
        table.insert(sData.player.killList, 1, {mob=mobName, xp=killXP, time=time()})
    ...
    -- on the XP value returned by the GetAverageBGsRemaining function.
    ...
    ---
	-- Mark that a quest was just turned in to attribute upcoming SP gains to quests.
	RecordQuestTurnIn = function(self)
        if sConfig.general.showDebug then
            DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00XToLevel: Quest Event Recorded (Window reset)|r")
        end
		self.soulData.lastQuestTime = time()
	end,

	---
	-- Called periodically to check if Ebonhold data changed.
	CheckSoulAshes = function(self)
        -- Debugging: Print status every 10 seconds if data is missing, to verify loop is running
        if time() - self.lastDebugTime > 10 then
            self.lastDebugTime = time()
            if not EbonholdPlayerRunData and sConfig.general.showDebug then
                DEFAULT_CHAT_FRAME:AddMessage("|cff888888XToLevel Debug: Polling... Ebonhold Data is NIL|r")
            end
        end

		if not EbonholdPlayerRunData or not EbonholdPlayerRunData.soulPoints then return end
		
		local actual = EbonholdPlayerRunData.soulPoints
		
		-- Initialize if first run
		if not self.soulData.startTime then
            if sConfig.general.showDebug then
                DEFAULT_CHAT_FRAME:AddMessage("|cff00FF00XToLevel: First Data received! SP: " .. actual .. "|r")
            end
			self.soulData.startTime = time()
			self.soulData.startAmount = actual
			self.soulData.currentAmount = actual
            self.soulData.active = true
			return
		end

		local diff = actual - self.soulData.currentAmount

		if diff > 0 then
            if sConfig.general.showDebug then
                DEFAULT_CHAT_FRAME:AddMessage("|cff00FF00XToLevel: SP Gained: " .. diff .. "|r")
            end
			self.soulData.currentAmount = actual
			self.soulData.sessionTotal = self.soulData.sessionTotal + diff
			
			-- Heuristic: If we turned in a quest in the last 10 seconds, assume it's Quest SP
            -- Increased window from 2s to 10s to account for server delay after Quest events
			if (time() - self.soulData.lastQuestTime) <= 10 then
                if sConfig.general.showDebug then
                    DEFAULT_CHAT_FRAME:AddMessage("|cff00FF00XToLevel: Attributed to Quest|r")
                end
				self.soulData.sessionQuests = self.soulData.sessionQuests + diff
			else
				-- Otherwise assume it is a Kill (or loose loot)
                if sConfig.general.showDebug then
                    DEFAULT_CHAT_FRAME:AddMessage("|cff00FF00XToLevel: Attributed to Kill/Loot|r")
                end
				self.soulData.sessionKills = self.soulData.sessionKills + diff
			end
			
			-- Trigger LDB update immediately
			XToLevel.LDB:Update()
		elseif diff < 0 then
            -- Debug negative diff
            if sConfig.general.showDebug then
                DEFAULT_CHAT_FRAME:AddMessage("|cffFF0000XToLevel: SP Spent/Lost: " .. diff .. "|r")
            end
			-- Spent points (died or bought skills), just reset current tracking point so we don't count negative gain
			self.soulData.currentAmount = actual
		end
	end,

	---
	-- Get Soul Points per Hour
	-- returns: totalPH, killsPH, questsPH
	GetSoulAshesPerHour = function(self)
		if not self.soulData.startTime then return 0, 0, 0 end
		
		local duration = time() - self.soulData.startTime
		if duration < 1 then duration = 1 end
		
		local totalPH = (self.soulData.sessionTotal / duration) * 3600
		local killsPH = (self.soulData.sessionKills / duration) * 3600
		local questsPH = (self.soulData.sessionQuests / duration) * 3600
		
		return math.floor(totalPH), math.floor(killsPH), math.floor(questsPH)
	end,
};
