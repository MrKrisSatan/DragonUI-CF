...
XToLevel.Tooltip = 
{
    ...
    Show = function(self, frame, anchorPont, relativeFrame, relativePoint, footerText, mode)
		...
		elseif mode == "timer" then
            ...
            -- Add Soul Ashes Section (Fallback for overall view)
            if XToLevel.Player.soulData.active then
                GameTooltip:AddLine(" ")
                GameTooltip:AddLine(L['Soul Ashes (Session)'])
                self:AddSoulAshes()
            end
    ...
    AddKillRange = function(self)
        ...
        -- Soul Ashes Info for Kills
        if XToLevel.Player.soulData and XToLevel.Player.soulData.active then
            local _, killPH = XToLevel.Player:GetSoulAshesPerHour()
            if killPH > 0 then
                GameTooltip:AddLine(" ")
                GameTooltip:AddDoubleLine(" " .. L['Soul Ashes'] .. " ("..L['Kills'].."):", XToLevel.Lib:NumberFormat(killPH) .. "/hr", self.labelColor.r, self.labelColor.g, self.labelColor.b, 1, 0.82, 0)
            end
        end
    end,
    ...
    AddQuestRange = function(self)
        ...
        -- Soul Ashes Info for Quests
        if XToLevel.Player.soulData and XToLevel.Player.soulData.active then
            local _, _, questPH = XToLevel.Player:GetSoulAshesPerHour()
            if questPH > 0 then
                GameTooltip:AddLine(" ")
                GameTooltip:AddDoubleLine(" " .. L['Soul Ashes'] .. " ("..L['Quests'].."):", XToLevel.Lib:NumberFormat(questPH) .. "/hr", self.labelColor.r, self.labelColor.g, self.labelColor.b, 1, 0.82, 0)
            end
        end
    end,
	...
	AddTimerDetailes = function(self, mininmal)
		if sConfig.timer.enabled then
            ...
            -- Soul Ashes Info for Timer (Total)
            if XToLevel.Player.soulData and XToLevel.Player.soulData.active then
                local totalPH = XToLevel.Player:GetSoulAshesPerHour()
                if totalPH > 0 then
                    if not mininmal then GameTooltip:AddLine(" ") end
                    GameTooltip:AddDoubleLine(" " .. L['Soul Ashes'] .. "/hr:", XToLevel.Lib:NumberFormat(totalPH), self.labelColor.r, self.labelColor.g, self.labelColor.b, 1, 0.82, 0)
                end
            end
		else
    ...
    ---
    -- Adds Soul Ashes info
    AddSoulAshes = function(self)
        local totalPH, killPH, questPH = XToLevel.Player:GetSoulAshesPerHour()
        local sessionTotal = XToLevel.Player.soulData.sessionTotal
        local sessionKills = XToLevel.Player.soulData.sessionKills
        local sessionQuests = XToLevel.Player.soulData.sessionQuests
        
        GameTooltip:AddDoubleLine(" " .. L['Total Gained'] .. ":", XToLevel.Lib:NumberFormat(sessionTotal), 1,1,1, 1,1,1)
        GameTooltip:AddDoubleLine(" " .. L['Total / Hr'] .. ":", XToLevel.Lib:NumberFormat(totalPH), 1,1,1, 0,1,0)
        GameTooltip:AddLine(" ")
        GameTooltip:AddDoubleLine(" " .. L['From Kills'] .. ":", XToLevel.Lib:NumberFormat(sessionKills) .. " ("..XToLevel.Lib:NumberFormat(killPH).."/hr)", 1,1,1, 0.7,0.7,0.7)
        GameTooltip:AddDoubleLine(" " .. L['From Quests'] .. ":", XToLevel.Lib:NumberFormat(sessionQuests) .. " ("..XToLevel.Lib:NumberFormat(questPH).."/hr)", 1,1,1, 0.7,0.7,0.7)
    end,
}