...
sConfig = {
	...
	timer = {
		enabled = true,
		mode = 1, -- 1 = session, 2 = level, 3 = kill range
...
XToLevel.Config =
{
	...
	--- Refreshes all visible addon components.
	RefreshAllDisplays = function(self)
		XToLevel.Average:Update()
		XToLevel.LDB:BuildAndCachePattern()
		XToLevel.LDB:Update()
	end,
    ...
    Initialize = function(self)
		...
		self:CreateCustomPatternHelpPanel(topPanel)
	    ...
	    StaticPopupDialogs['XToLevelConfig_ResetPlayerKills'] = {
			...
			OnAccept = function()
	            ...
	            XToLevel.Config:RefreshAllDisplays();
			end,
			timeout = 0,
			whileDead = true,
			hideOnEscape = true,
	...
	panel_alias = {
		...
		["customhelp"] = "XToLevel_CustomHelpPanel",
    ...
    CreateMainPanel = function(self)
        ...
        mainPanel.childFrame["AboutFrame"]:SetHeight(120)
        ...
        self:CreateTextLine(mainPanel.childFrame["AboutFrame"], "Version", L["Version"], "3.3.3_15r|r |cFFAAFFAA(2026-01-23)|r |cffff0000 by Skulltrail (with Project Ebonhold compatibility)", "00FF00")
		...
		self:CreateTextLine(mainPanel.childFrame["AboutFrame"], "Contributors", L["Contributors"], "Skulltrail - 3.3.3_15r|r |cFFAAFFAA(2026-01-23) (with Project Ebonhold compatibility)", "FFFFFF")
    ...
    CreateLDB = function(self, parent)
        ...
        self:CreateH2(ldbPanel, "SoulAshesHeader", "Soul Ashes (Project Ebonhold)")
        self:CreateCheckbox(ldbPanel, "ShowSPH", L["Show Soul Ashes / Hour"],
            function(self) self:SetChecked(sConfig.ldb.text.sph) end,
            function(self) sConfig.ldb.text.sph = self:GetChecked() or false end)
        self:CreateCheckbox(ldbPanel, "ShowSPTotal", L["Show Session Total SP"],
            function(self) self:SetChecked(sConfig.ldb.text.sp_total) end,
            function(self) sConfig.ldb.text.sp_total = self:GetChecked() or false end)
	...
	---
	-- Creates the help panel for Custom LDB patterns.
	CreateCustomPatternHelpPanel = function(self, parent)
		local height = 650
		local helpPanel = self:CreatePanel("XToLevel_CustomHelpPanel", "Custom LDB Patterns", height, parent)
		
		local helpText1 = "The following describes how to use custom patterns in the XToLevel LDB text output.\n\n" ..
		"|cFFFFD100Note!|r Storing your patterns in a safe place outside the game is a good idea. Patterns |cFFFF5555might|r be lost during patches and addon updates."
		self:CreateDescription(helpPanel, "CustomHelpIntro", helpText1, 50, "FFFFFF")

		self:CreateH2(helpPanel, "SimpleHeader", "The Simple Version")
		local helpText2 = "This is a simplified version, which is good for creating simple patterns.\n\n" ..
		"|cFFFFD100Available tags:|r\n" ..
		"|cFFEEEEEE<kills>|r, |cFFEEEEEE<quests>|r, |cFFEEEEEE<bgs>|r, |cFFEEEEEE<bgo>|r, |cFFEEEEEE<xp>|r, |cFFEEEEEE<xpnum>|r, |cFFEEEEEE<rested>|r, |cFFEEEEEE<restedp>|r, |cFFEEEEEE<pet>|r, |cFFEEEEEE<petxp>|r, |cFFEEEEEE<petxpnum>|r, |cFFEEEEEE<timer>|r\n\n" ..
		"|cFFFFD100Each tag can have attributes:|r\n" ..
		"|cFFEEEEEElabel|r - A string to be added in front of the tag's value.\n" ..
		"|cFFEEEEEEseperator|r - A character to be used as a separator between tags.\n" ..
		"|cFFEEEEEEpost|r - A character to be added after the value.\n\n" ..
		"|cFFFFD100Examples:|r\n" ..
		"|cFFDDDDDD<kills label=\"\" post=\"K\"><quests label=\"\" post=\"Q\" seperator=\" \">|r\n" ..
		"|cFFEEEEEEOutput:|r 100K 20Q\n" ..
		"|cFFDDDDDD<pet label=\"Pet Kills\" post=\"-\"> [<petxp label=\"\" post=\"-\">]|r\n" ..
		"|cFFEEEEEEOutput:|r Pet Kills: 100- [25%-]"
		self:CreateDescription(helpPanel, "CustomHelpSimple", helpText2, 220, "FFFFFF")

		self:CreateH2(helpPanel, "AdvancedHeader", "The Complete Version")
		local helpText3 = "The patterns use a very basic HTML-like syntax, using {} rather than <>.\n\n" ..
		"|cFFFFD100Available tags:|r\n" ..
		"|cFFEEEEEE{kills}|r, |cFFEEEEEE{quests}|r, |cFFEEEEEE{bgs}|r, |cFFEEEEEE{bgo}|r, |cFFEEEEEE{xp}|r, |cFFEEEEEE{xpnum}|r, |cFFEEEEEE{pet}|r, |cFFEEEEEE{petxp}|r, |cFFEEEEEE{petxpnum}|r, |cFFEEEEEE{timer}|r\n\n" ..
		"|cFFFFD100Inside each tag, you can use:|r\n" ..
		"|cFFEEEEEE{$label}|r - The locale-specific label.\n" ..
		"|cFFEEEEEE{$value}|r - The tag's calculated value.\n" ..
		"|cFFEEEEEE{$seperator:X}|r - A separator character.\n\n" ..
		"|cFFFFD100Styling tags:|r\n" ..
		"|cFFEEEEEE{color=RRGGBB}text{/color}|r\n" ..
		"|cFFEEEEEE{progress}text{/progress}|r\n\n" ..
		"|cFFFFD100Example:|r\n" ..
		"|cFFDDDDDD{kills}{$seperator: }{color=FF0000}{$label}:{/color} {progress}{$value}{/progress}{/kills}|r"
		self:CreateDescription(helpPanel, "CustomHelpAdvanced", helpText3, 210, "FFFFFF")
	...
	CreateTimerPanel = function(self, parent)
        ...
        self:CreateCheckbox(timerPanel, "TimerEnable", L["Enable timer"] or "Enable timer",
            ...
            function(self) 
				...
				if sConfig.timer.enabled then
					...
					if XToLevel.Player.timerHandler then
						XToLevel.timer:CancelTimer(XToLevel.Player.timerHandler)
						XToLevel.Player.timerHandler = nil
					end
				end
		...
		desc = desc .."|cFFFFD100Session:|r Uses only the XP gained since the UI was loaded. Ideal as a \"real-time\" estimate while farming.\n"
		desc = desc .."|cFFFFD100Level:|r Uses the total time and XP this level. Gives a better long-term estimate for quest and dungeon runners. (Note that this may be fairly inaccurate during the first few % of a new level.)\n"
		desc = desc .."|cFFFFD100Kill Range:|r Provides a short-term estimate based on your last few kills, perfect for grinding."
		
		self:CreateH2(timerPanel, "TimerModeHeader", L["Mode"] or "Mode")
		self:CreateDescription(timerPanel, "TimerModeDescriotion", desc, 88, "FFFFFF")
		self:CreateSelectBox(timerPanel, "TimerModeSelect", {"Session", "Level", "Kill Range"}, "Session",
            -- OnShow
            function()
				local chosenType
				if sConfig.timer.mode == 1 then chosenType = "Session"
				elseif sConfig.timer.mode == 2 then chosenType = "Level"
				elseif sConfig.timer.mode == 3 then chosenType = "Kill Range"
				else chosenType = "Session" end
                UIDropDownMenu_SetSelectedName(this, chosenType, true);
                UIDropDownMenu_SetText(this, chosenType);
            end,
            -- OnChange
            function(selectBox)
				...
				if chosen == "Session" then sConfig.timer.mode = 1
				elseif chosen == "Level" then sConfig.timer.mode = 2
				elseif chosen == "Kill Range" then sConfig.timer.mode = 3
				else sConfig.timer.mode = 1 end
				XToLevel.Average:Update()
            end
        )
		...
		self:CreateCheckbox(timerPanel, "TimerModeFallback", L['TimerModeFallback'] or "Fall back on \"Level\" if session data is not available",
            function(self) self:SetChecked(sConfig.timer.allowLevelFallback)  end,
            function(self) sConfig.timer.allowLevelFallback = self:GetChecked() or false end)
    ...
    CreateColorPicker = function(self, parent, fieldName, label, initalColor, onChange)
    	...
    	_G[parent.childFrame[fieldName]:GetName() .. "Text"]:SetText(label)
    	_G[parent.childFrame[fieldName]:GetName() .. "Color"]:SetTexture(unpack(initalColor or {1, 1, 1, 1}))
...
function XToLevel.Config:Verify()
    ...
    if sConfig.ldb.text.sph == nil then sConfig.ldb.text.sph = true end
    if sConfig.ldb.text.sp_total == nil then sConfig.ldb.text.sp_total = false end
